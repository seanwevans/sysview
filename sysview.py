...
class SyscallConfig:
    ...
    def get_enabled_syscalls(self):
        ...
        return {name: config for name, config in self.syscalls.items() 
...
class SyscallMonitor:
    ...
    def initialize_data_structures(self):
        ...
        for name, config in self.config.get_enabled_syscalls().items():
            ...
            self.history[name] = collections.deque([0] * self.history_size, maxlen=self.history_size)
...
class CursesDisplay:
    ...
    def display_live_view(self, stdscr, current_rates):
        ...
        for syscall in self.monitor.syscalls:
            name = syscall["name"]
            max_rates[name] = max(self.monitor.history[name]) if any(self.monitor.history[name]) else 1
    ...
    def display_summary(self, stdscr):
        ...
        sorted_syscalls = sorted(
            [(syscall["name"], self.monitor.total_counts[syscall["name"]], self.monitor.peak_rates[syscall["name"]]) 
             for syscall in self.monitor.syscalls],
            key=lambda x: x[1],
            reverse=True,
        )
        ...
        if total_count > 0:
            ...
            for i, (name, count, _) in enumerate(sorted_syscalls):
                if count > 0:
                    ...
                    pct_line = (
                        f"{name:8s}: {percent:5.1f}% ({self.format_number(count)} calls) "
                    )
...
def main():
    ...
    parser.add_argument("--history", "-H", type=int, default=60, help="History size (number of samples to display)")
    parser.add_argument("--output", "-o", help="Save results to file (JSON)")
    parser.add_argument("--generate-config", "-g", help="Generate default config file and exit")
